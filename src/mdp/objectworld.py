...
"""
Implementation of the objectworld MDP in Levine et al. 2011. This code is used and adapted from
...
class Objectworld(Gridworld):
    ...
    def __init__(self, grid_size, n_objects, n_colours, wind, discount):
        ...
        self.transition_probability/=np.sum(self.transition_probability, axis=2, keepdims=True)
        #print(np.sum(self.transition_probability, axis=2).tolist())
        #assert(np.all(np.sum(self.transition_probability, axis=2)==1.0))
    ...
    def collect_demonstrations(self, n_trajectories, trajectory_length):
      ...
      action_probabilities = np.zeros((self.n_states, self.n_actions))
      for traj in trajectories:
          for (s, a, r, ns) in traj:
              action_probabilities[s][a] += 1
      action_probabilities[action_probabilities.sum(axis=1) == 0] = 1e-5
      action_probabilities /= action_probabilities.sum(axis=1).reshape(self.n_states, 1)
